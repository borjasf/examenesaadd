...
public class FactoriaRepositorios {
	...
	@SuppressWarnings("unchecked")
	public static <R extends RepositorioTablonesAdHoc> R getRepositorioTablonesAdHoc(Class<?> entidad) {
		try {				
			PropertiesReader properties = new PropertiesReader(PROPERTIES);		
			String clase = properties.getProperty(entidad.getName());
			return (R) Class.forName(clase).getConstructor().newInstance();
			...
			e.printStackTrace();
			throw new RuntimeException("No se ha podido obtener el repositorio adhoc para la entidad: " + entidad.getName());
		}
	}
